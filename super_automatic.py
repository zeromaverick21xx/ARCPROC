...
# Changed code to work with changes in DP_DELIVERY_STATUS and DP_DELIVERY_ASDM_OUS performede on 2015-10-21
# 2015-10-22 works with changes in the database DP_DELIVERY tables 
# 2015-10-24 validates if the tar is 0 before ingesting
# 2015-12-01 Add new version of check README file
...
def checkReadme(tar_file_path):
    ...
    if not files_names_readme:
        GLOBAL_ERROR = 'README error: README file has not a directory tree or has wrong character format - Please ask ARCs to resend the files'
        return False   
    not_found = list(set_names_readme - set_names_tar)
    if not_found:
        GLOBAL_ERROR = 'README error: The following files and/or directories are not present in the tar file structure: ' + '\n'.join(not_found)
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        ...
        print ""
        print "               reingestion_automatic.py <PROJECT_MOUSS_PARTS>.tar"
        print ""
        sys.exit()
    else:      
        delivery = sys.argv[1]
...
#           print row[0]
                print ''
                print "=============================================="
                print "This is a NEW tar, please use all_automatic.py"
                print "=============================================="
                print ''
                #sys.exit('Exiting')
                break
...
#           print row[0]
                ...
                print ""
                print "please check there are no duplicates in DP_DELIVERY_STATUS " 
                print "================================================================="
                print ''
                break
    print ''
    apo = 'SACM-1029' 
#    apo = raw_input("Enter the number of the APO Reingestion - i.e. 758 - : ")
#   sys.exit('Exiting')
    ...
    if not checkReadme(full_name):
        print GLOBAL_ERROR
        sys.exit()
    print 'Readme file is Ok'
    #cycle1Tar(full_name)
    data_ingest(full_name)

# DELETE RELATED DP_DELIVERY TABLES
    f=open('temp_reingestion_query_for_'+full_name+'.sql','w')
    f.write("delete from alma.dp_delivery_asdm_ous where DELIVERABLE_NAME like '"+delivery+"%';\n")
    f.write("commit;\n")
    f.write("exit\n")
    print '__________________________________________________________________________________________________'
    print ''
    print 'DP_DELIVERY_ASDM rows for '+delivery+' to be deleted'
    print '__________________________________________________________________________________________________'
    print ''
    passwd_os=passwd.replace('$','\$')
    g=open('temp_reingestion_script_'+full_name+'.sh','w')
    script_query='temp_reingestion_script_'+full_name+'.sh'
    g.write("sqlplus alma/"+passwd_os+"@'ALMA_OFFLINE.SCO.CL' @temp_reingestion_query_for_"+full_name+".sql\n")
    f.close()
    g.close()
    call("sh '%s'"%script_query, shell=True) # uncomment this line
    print ""
    print "Rows in DP_DELIVERY_ASDM_OUS deleted successfully"
    print ""
    print '__________________________________________________________________________________________________'
#END OF DELETE RELATED DP_DELIVERY TABLES
    ...
    sysdate_year = row[8]
    print ''
    f.write("update alma.dp_delivery_status set release_date_comment='Reingestion: APO-"+apo+"' where delivery_id='"+delivery+"';\n")
